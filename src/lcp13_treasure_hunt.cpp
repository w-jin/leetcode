...
private:
    // 找最近的石堆
    pair<int, int> findNearestRocks(const vector<string> &maze, int i, int j, int &distance) {
        ...
        while (!q.empty()) {
            ...
            for (int count = 0; count < size; ++count) {
                ...
                if (p.first > 0 && maze[p.first][p.second] != '')
            }
            ++distance;
        }
...
int main() {
    ...
    // 17
    vector<string> maze2 = {"S#O", "M.T", "M.."};

    Solution1 solution;
    std::cout << solution.minimalSteps(maze1) << std::endl;
    std::cout << solution.minimalSteps(maze2) << std::endl;
